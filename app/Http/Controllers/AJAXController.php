...
class AJAXController extends Controller
{
    ...
    public function getHotel(Request $request)
    {
        ...
        if ($field != "none" && $order != "none" && $filter != "none") {
            ...
            if ($kotaId == 'all' && $provinsiId == 'all') {
                $alamat = alamat::select('alamat.hotel_id as hotel_id','hotel.photo as photo','hotel.name as name','hotel.star as star','hotel.total_rating as rating','hotel.reviewers as reviewers','alamat.detailLengkap as detailLengkap','kota.namaKota as namaKota','provinsi.namaProvinsi as namaProvinsi','room_details.cost as cost')->join('hotel', 'alamat.hotel_id', '=', 'hotel.id')->join('kota','alamat.kota_id','=','kota.id')->join('provinsi','alamat.provinsi_id','=','provinsi.id')->join('room_details','hotel.id','=','room_details.hotel_id')->groupBy('alamat.hotel_id','hotel.photo','hotel.name','hotel.star','hotel.total_rating','hotel.reviewers','alamat.detailLengkap','kota.namaKota','provinsi.namaProvinsi')->orderBy($field,$order)->get();
            } elseif ($kotaId == 'all') {
                $query = ['alamat.provinsi_id' => $provinsiId];
                $alamat = alamat::select('alamat.hotel_id as hotel_id','hotel.photo as photo','hotel.name as name','hotel.star as star','hotel.total_rating as rating','hotel.reviewers as reviewers','alamat.detailLengkap as detailLengkap','kota.namaKota as namaKota','provinsi.namaProvinsi as namaProvinsi','room_details.cost as cost')->where($query)->join('hotel', 'alamat.hotel_id', '=', 'hotel.id')->join('kota','alamat.kota_id','=','kota.id')->join('provinsi','alamat.provinsi_id','=','provinsi.id')->join('room_details','hotel.id','=','room_details.hotel_id')->groupBy('alamat.hotel_id','hotel.photo','hotel.name','hotel.star','hotel.total_rating','hotel.reviewers','alamat.detailLengkap','kota.namaKota','provinsi.namaProvinsi')->orderBy($field,$order)->get();
            } elseif ($provinsiId == 'all') {
                $query = ['kota_id' => $kotaId];
                $alamat = alamat::select('alamat.hotel_id as hotel_id','hotel.photo as photo','hotel.name as name','hotel.star as star','hotel.total_rating as rating','hotel.reviewers as reviewers','alamat.detailLengkap as detailLengkap','kota.namaKota as namaKota','provinsi.namaProvinsi as namaProvinsi','room_details.cost as cost')->where($query)->join('hotel', 'alamat.hotel_id', '=', 'hotel.id')->join('kota','alamat.kota_id','=','kota.id')->join('provinsi','alamat.provinsi_id','=','provinsi.id')->join('room_details','hotel.id','=','room_details.hotel_id')->groupBy('alamat.hotel_id','hotel.photo','hotel.name','hotel.star','hotel.total_rating','hotel.reviewers','alamat.detailLengkap','kota.namaKota','provinsi.namaProvinsi')->orderBy($field,$order)->get();
            } else {
